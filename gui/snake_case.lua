...
function finity.new(isdark, gprojectName, thinMenu)
	...
	self2.change_toggle_key = function(NewKey)
		finity_data.toggle_key = NewKey
		
		if not projectName then
			self2.tip.Text = "Press '".. string.sub(tostring(NewKey), 14) .."' to hide this menu"
		end
		
		if finity_data.UpConnection then
			finity_data.UpConnection:Disconnect()
		end

		finity_data.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input)
			if Input.KeyCode == finity_data.toggle_key and not typing then
                toggled = not toggled
	...
	self2.change_background_image = function(ImageID, Transparency)
	...
	self2.tip = self:Create("TextLabel", {
		...
		Text = "Press '".. string.sub(tostring(self.toggle_key), 14) .."' to hide this menu",
	...
	if projectName then
		...
		self2.tip.Text = "Press '".. string.sub(tostring(self.toggle_key), 14) .."' to hide this menu"
	end

    self2.set_name = function(text)
	...
	function self2:category(name)
		...
		function category.set_name(name)
		...
		function category:sector(name)
			...
			function sector:cheat(kind, name, callback, data)
				...
				if kind then
					if string.lower(kind) == "checkbox" or string.lower(kind) == "toggle" or string.lower(kind) == "check_box" then
						if data then
						...
						function cheat:set_value(value)
                            cheat.value = value
                    ...
                    elseif string.lower(kind) == "color" or string.lower(kind) == "colorpicker" or string.lower(kind) == "color_picker" then
					...
					elseif string.lower(kind) == "dropdown" or string.lower(kind) == "drop_down" then
						if data then
						...
						function cheat:remove_option(value)
							local removed = false
						...
						function cheat:add_option(value)
						...
						function cheat:get_options()
							return options
						end

						function cheat:refresh_options()
							refreshOptions()
						end
					...
					elseif string.lower(kind) == "textbox" or string.lower(kind) == "text_box" then
					...
					elseif string.lower(kind) == "keybind" or string.lower(kind) == "bind" or string.lower(kind) == "key_bind" then
						...
						cheat.button.MouseButton1Up:Connect(function()
							...
							connection = finity.gs["UserInputService"].InputBegan:Connect(function(Input)
								if Input.UserInputType.Name == "Keyboard" and Input.KeyCode ~= finity_data.toggle_key and Input.KeyCode ~= Enum.KeyCode.Backspace then
								...
								elseif Input.KeyCode == Enum.KeyCode.Backspace then
									...
									connection:Disconnect()
									connection = nil
								elseif Input.KeyCode == finity_data.toggle_key then
